...
public class UI {
	...
	//public static final int flagSDP=1<<29;   //鍙戦�佹椂寤剁粰瀵规柟鐨勫崗璁紝鎺ユ敹鍒版鍗忚鏃跺紑鍚疭endDelayToOther.sendToOther()绾跨▼
	...
	public static void writeCSV(){
		...
		try {
        	...
        	for(int i=0;i<Link.SendPackages.size();i++) {
        		...
        		bw.append(temp.getTime()+","+temp.getEquip()+","+temp.getCCID()+","+temp.getMsgID()+","+temp.getDataLen());
        		bw.newLine();
        	}
        	
        	for(int i=0;i<Link.ReceivePackages.size();i++) {
        		RecvInfo temp=Link.ReceivePackages.get(i);
        		bw.append(temp.getTime()+","+temp.getEquip()+","+temp.getCCID()+","+temp.getRecvID()+","+""+","+Link.delayMill.get(i));
        		bw.newLine();
        	}
