...
public class Result {
	...
	private Text txtSendBps;
	private Text txtRevBps;
	...
	public void showResult(final ArrayList<Double> allDelay,final long runTime,Link link,final int Im) {
		...
		shlCbtcDataStream.setSize(956, 711);
		...
		txtSendBps = new Text(shlCbtcDataStream, SWT.BORDER);
		txtSendBps.setText("0");
		txtSendBps.setEditable(false);
		txtSendBps.setBounds(760, 551, 123, 30);
		txtSendBps.setText(Integer.toString(SendThread.num1*8));
		if(UI.Im==0){
			txtSendBps.setText(Double.toString((int)(SendThread.num1*8/(double)runTime*1000)));
		}else {
			txtSendBps.setText(Double.toString((int)(SendThread.num2*8/(double)runTime*1000)));
		}


		Label lblSendBps = new Label(shlCbtcDataStream, SWT.NONE);
		lblSendBps.setBounds(760, 523, 123, 24);
		lblSendBps.setText("Send(bps)");

		Label lblRevBps = new Label(shlCbtcDataStream, SWT.NONE);
		lblRevBps.setText("Receive(bps)");
		lblRevBps.setVisible(false);
		lblRevBps.setBounds(760, 593, 123, 24);

		txtRevBps = new Text(shlCbtcDataStream, SWT.BORDER);
		txtRevBps.setText("0");
		txtRevBps.setEditable(false);
		txtRevBps.setBounds(760, 619, 123, 30);
		txtRevBps.setVisible(false);
//		txtRevBps.setText(Double.toString(RecvThread.revBps*8/(double)runTime*1000));
//		txtRevBps.setText(Double.toString((int)(SendThread.num2*8/(double)runTime*1000)));
